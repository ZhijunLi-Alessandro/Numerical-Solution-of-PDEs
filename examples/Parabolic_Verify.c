...
int main(){
    ...
    double tau = grid->hx * grid->hx * grid->hy * grid->hy / (grid->hx * grid->hx + grid->hy * grid->hy) / 4 * 12;
    ...
    write_csv_int_matrix("results/Parabolic/data/exact_test/grid_data.csv", grid->region, grid->nx, grid->ny);
    ...
    while (t_now < T_max) {
        ...
        for (int i = 0; i < grid->n_active; i++) {
            ...
            if (grid->region[gi][gj] == 1) {
                rhs[i] = (exact[i] - compute[i]) * grid->hx * grid->hy / tau;
            }
            else {
                rhs[i] = 0;
            }
        ...
        if ((step % output_interval) == 0) {
            ...
            sprintf(fname_exact, "results/Parabolic/data/exact_test/exact_%06d.csv", step);
            char fname_rhs[256];
            sprintf(fname_rhs, "results/Parabolic/data/exact_test/rhs_%06d.csv", step);
